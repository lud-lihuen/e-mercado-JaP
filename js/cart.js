...
let medioPagoFeedback = document.getElementById("medioPago-feedback");
let subtotal = 0;
let shippingCost = 0;
let total = 0;
...
function deleteArticle(article) {
    ...
    if (Object.keys(cart).toString().indexOf(cartUser) === -1) {
        showEmptyCart();
    } else {
...
// Funcion que valida medio de pago:
function validatePaymentMethod() {
    if (form2.checkValidity()) {
        medioPagoFeedback.style.display = 'none';
    } else {
        medioPagoFeedback.style.display = 'block';
    }
}

// Funcion que valida formularios al finalizar compra:
...
document.addEventListener("DOMContentLoaded", function () {
    if (Object.keys(cart).toString().indexOf(cartUser) === -1) {
        ...
        document.getElementsByName("medioPago").forEach(function (radio) {
            radio.addEventListener("change", function () {
                ...
                if (document.getElementById("transferencia").checked) {
                    ...
                    document.getElementById("paymentMethod").innerHTML = `Transferencia bancaria`;
                }
            })
        });
        ...
        form2.addEventListener("submit", function (event) {
            ...
            document.getElementsByName("datosMedioPago").forEach(function (input) {
